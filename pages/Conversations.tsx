...
const MOCK_CHATS: Conversation[] = [
  { id: '1', contact: { id: 'c1', name: 'Ana Souza', phone: '11999999999', status: 'active', lastMessageAt: '10:30' }, lastMessage: 'Gostaria de negociar o boleto.', unreadCount: 2, status: 'ai', tags: ['financeiro'], updatedAt: '2023-10-27T10:30:00' },
  { id: '2', contact: { id: 'c2', name: 'Carlos Lima', phone: '11988888888', status: 'active', lastMessageAt: '09:15' }, lastMessage: 'Já realizei o pagamento via PIX.', unreadCount: 0, status: 'human', tags: ['suporte'], updatedAt: '2023-10-27T09:15:00' },
  { id: '3', contact: { id: 'c3', name: 'Tech Solutions Ltda', phone: '11977777777', status: 'active', lastMessageAt: 'Ontem' }, lastMessage: 'Boleto vencido enviado.', unreadCount: 0, status: 'ai', tags: ['cobrança'], updatedAt: '2023-10-26T14:20:00' },
];
...
const Conversations = () => {
  const [selectedChat, setSelectedChat] = useState<Conversation | null>(MOCK_CHATS[0]);
  ...
  const toggleMode = () => {
    ...
    // In a real app, we would make an API call to update the conversation status
  };

  return (
    <div className="flex h-[calc(100vh-2rem)] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      {/* List (Left) */}
            ...
            <div className="p-4 bg-white border-t border-slate-200">
                {selectedChat.status === 'ai' ? (
                     <div className="flex items-center justify-between bg-indigo-50 border border-indigo-100 rounded-lg p-4">
                        ...
                        <button 
                            onClick={toggleMode}
                            className="bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm"
                        >
                            Assumir conversa
                        </button>
                     </div>
